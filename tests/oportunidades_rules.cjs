...
function ready(p){const a=p.anuncios[0];a.vendedor='LOJA1';a.vendas_sem=12;a.revisadoEm='2026-09-09';p.anuncios.push({...a,id:'MLB2',vendedor:'LOJA2'});p._coverage={buscado:true,paginacaoCompleta:true,candidatos:2,revisados:2,lidoEm:'2026-09-09'};return p;}
let r=api.analyze(ready(product()),supplier,env);assert.equal(r.status,'Promissor');assert.ok(Math.abs(r.classic.profit-18.97925)<1e-6);assert.equal(r.capital,2820);assert.equal(r.demand.daily,1);assert.equal(r.entrance,'disputada');assert.equal(r.market.strong,2);assert.equal(r.market.growing,2);
...
r=api.analyze(ready(doll),{regra:'Loja física, aceita quantidade menor que a caixa fechada'},env);assert.equal(r.status,'Promissor');assert.equal(r.approximate,true);assert.equal(api.analyze(doll,supplier,env).status,'Promissor'); // João autorizou similares também em caixa, com condição.
assert.equal(api.analyze(product({catalogo:true,bb:6,preco_min:61.49}),supplier,env).status,'Não vale o teste');
assert.equal(api.analyze(ready(product({catalogo:true,bb:2,preco_min:61.49})),supplier,env).status,'Promissor');
assert.equal(api.analyze(product({qtd:null}),supplier,env).classic,null);
let p=product();p.anuncios.push({...p.anuncios[0],id:'MLB2',vendas_mes:30,preco:59});r=api.analyze(p,supplier,env);assert.equal(r.monthly,40);assert.equal(r.ads.length,2);assert.equal(r.status,'Precisa de mais análise');
...
assert.equal(chosen.status,'Precisa de mais análise');assert.equal(chosen.ref.id,'NEW');assert.equal(chosen.leader.id,'OLD');assert.equal(chosen.monthly,40);assert.equal(chosen.evaluated.length,2);
mixed=product({id:'CAT',catalogo:true,bb:20,preco_min:20,vendas_mes:3000});
mixed.anuncios.push({...product().anuncios[0],id:'OWN'});
chosen=api.analyze(mixed,supplier,env);assert.equal(chosen.ref.id,'OWN');assert.equal(chosen.status,'Precisa de mais análise');
...
assert.equal(api.analyze(product({criadoEm:'2026-07-27',vendas_desde_criacao:44}),supplier,env).demand.age,44);
...
// Uma estrela isolada, uma busca parcial ou pouco crescimento nunca viram Promissor.
let isolated=product({vendedor:'LOJA1',vendas_sem:20});isolated._coverage={buscado:true,paginacaoCompleta:true,candidatos:4,revisados:1};
assert.equal(api.analyze(isolated,supplier,env).status,'Precisa de mais análise');assert.match(api.analyze(isolated,supplier,env).reason,/Demanda ainda não repetida/);
let partial=ready(product());partial._coverage.paginacaoCompleta=false;partial._coverage.candidatos=35;partial._coverage.revisados=3;
assert.equal(api.analyze(partial,supplier,env).status,'Precisa de mais análise');assert.match(api.analyze(partial,supplier,env).reason,/busca parcial/i);
let flat=ready(product());flat.anuncios.forEach(a=>a.vendas_sem=9);
assert.equal(api.analyze(flat,supplier,env).status,'Precisa de mais análise');assert.match(api.analyze(flat,supplier,env).reason,/Crescimento forte/);
