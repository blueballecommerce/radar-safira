...
| Hora | Modo | O que faz |
|---|---|---|
| **5h** | `novos` | lê os 100 mais vendidos e os 100 anúncios com até 90 dias no ar que já vendem, em cada uma das 27 categorias do ML (54 consultas); relê pelo id **tudo que estava na página e não reapareceu** na descoberta; dá nota a tudo e reordena. É o que traz produto novo. Termina por volta das 6h10: a JoomPulse só aceita **~40 consultas por hora**, então a sessão faz 36, espera a hora virar e continua. |
| **15h** | `atualiza` | relê pelo id **todos os produtos da página** (uns 4.500, em lotes de 100: ~45 consultas): vendas da semana e do mês, preço, dias no ar, quantos vendedores disputam o catálogo, avaliações. Reordena. Não traz produto novo. Termina por volta das 16h10, pelo mesmo motivo. |
...
em "Scheduled" na barra lateral: `radar-novos-5h` e `radar-atualiza-15h`. As permissões que elas usam (conector JoomPulse e o atalho
...
(`RADAR_MCP_MAX_POR_HORA`) e, quando acaba, manda a sessão esperar (`radar-mcp.cmd esperar`,
que dorme até a hora cheia seguinte) e continuar a mesma rodada. Por isso cada rodada leva uma
hora e pouco. Se um dia o plano da JoomPulse mudar, é só ajustar o número — com 60 por hora a
rodada termina em uns 15 minutos.
...
**Promissor** exige margem mínima de 20% no Clássico, criação e demanda comprovadas, entrada viável, pesquisa concluída e repetição do sinal em pelo menos **dois vendedores diferentes** com produto visualmente comparável. Cada referência precisa ter menos de 45 dias e média mínima de uma venda por dia. Pelo menos dois anúncios também precisam mostrar crescimento forte: o ritmo da última semana projetado para um mês deve superar a estimativa mensal do próprio anúncio em 20% ou mais. Essa comparação é um sinal de aceleração, não histórico. Catálogo com 6 ou mais vendedores nunca é Promissor; de 3 a 5 a entrada é disputada. Para catálogo aberto, a conta precisa permitir ficar abaixo do piso com 20%. Margem zero/negativa ou demanda comprovadamente insuficiente descartam aquela referência. Um único vencedor, uma busca parcial ou crescimento não demonstrado deixam **Precisa de mais análise**. Os descartados aparecem em **Mostrar tudo**.

Data de criação e dias de atividade são diferentes. A aba usa `criadoEm`/`adPublishDate`, ou `pub` de `data.json` ligado pelo mesmo MLB; jamais calcula criação subtraindo dias de atividade. A média usa uma estimativa de vendas desde a criação. Quando só existe uma janela de 30 ou 7 dias menor que a idade, calcula um **limite inferior** da média: isso pode comprovar a meta, mas não comprova fracasso se ficar abaixo dela. Vendas de anúncios/variações não são somadas. Variações do mesmo vendedor contam como um único sinal de mercado. Falta de dados aparece como **não coletado**. Uma única observação não demonstra estabilidade.
...
**Correções visuais:** Pulverizador × MLB4552614487 e Bolsa Transversal 3 Zíperes × MLB7344028306 foram confirmados como diferentes por João. A bolsa do anúncio é uma peça tática retangular, com construção e recursos superiores ao modelo triangular simples do fornecedor; ela não conta como prova de demanda ou margem. Os quatro pares em `data/oportunidades/pendentes.json` aguardam confirmação individual. Depois de confirmar, registrar decisões pelo modo direto e regenerar `python -m radar.fornecedor_export`. Não alterar os JSONs de `docs` à mão. A quantidade do kit de munição também permanece pendente e impede classificá-lo como Promissor.
...
A área de cobertura informa tamanho do catálogo, pesquisa anterior, produtos incluídos na busca recente e comparações revisadas. Cada produto distingue busca não realizada, busca parcial com mais páginas e busca concluída para os termos. Na ficha, candidatos recentes encontrados por texto aparecem em uma seção própria até serem comparados visualmente; mostram preço, demanda, criação, Full, catálogo e avaliações quando coletados, mas não entram na margem, na repetição nem na classificação. Correspondência textual apenas levanta candidatos. Resultado vazio de uma amostra não significa ausência de demanda.
...
Reaplicando a regra em 09/09/2026: **nenhum produto está Promissor**, 171 estão em **Precisa de mais análise** e 23 em **Não vale o teste** no universo com alguma referência; os demais não têm referência elegível. Repetidor e Cercado têm apenas um vendedor recente forte. Caneta 3D tem dois vendedores recentes fortes, mas só um anúncio com crescimento mínimo confirmado e a busca segue parcial: 3 de 35 candidatos foram comparados visualmente. Bolsa Térmica tem dois vendedores fortes, mas apenas um em crescimento e os concorrentes revisados têm recursos diferentes. Bolsa Transversal deixou a lista porque o único pareamento recente foi confirmado como produto diferente. A pesquisa parcial não permite concluir que todo o restante do catálogo seja inviável.

Na ficha, **Pesquisar no Mercado Livre** abre uma nova aba com o nome do produto já aplicado à busca geral do marketplace. O objetivo é permitir que João veja outras ofertas e vendedores além dos anúncios já registrados no Radar. O painel **Categorias possíveis** reúne apenas a categoria associada pelo Radar e as categorias encontradas nos anúncios recentes conferidos como iguais ou parecidos. As opções são agrupadas pela categoria principal; quando há caminho mais detalhado, ele aparece completo. Cada opção mostra as comissões atuais de Clássico e Premium da mesma tabela `FEES` usada pelo Simulador. Categoria ausente aparece como não coletada; a tela não cria sugestão sem evidência. Comissão por categoria não inclui custo fixo, imposto, frete ou embalagem.
