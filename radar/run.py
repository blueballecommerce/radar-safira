...
class FixtureSource(Source):
    ...
    async def products_by_ids(self, ids):
        return []  # fixtures não têm releitura por id
...
async def run_once(src: Source, db: DB, *, force_categories: bool = False, force_joompro: bool = False,
    ...
    try:
        ...
        db.finish_run(run_id, "ok", len(products), src.calls, f"tracked={len(to_track)} missing={missing} month={month}")
        db.commit()
        out = export_json(db)
        return {"run_id": run_id, "products": len(products), "ranked": len(live), "queries": src.calls,
                "tracked": len(to_track), "missing": missing, "month": month, "export": str(out)}
