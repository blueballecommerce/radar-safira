...
| Arquivo | O que é |
|---|---|
| `index.html` (~1.500 linhas) | **A página inteira**: abas Oportunidades ML, Categorias em alta, Importação JoomPro (vazia), Simulador de preço, Fornecedores, Shopee (em breve). Constantes no começo do JavaScript, perto da linha 556: `NEW_MAX_DAYS = 30` (o que é "novo") e `HIDE_RULES` (nichos escondidos; botão "Mostrar tudo"). Busca `data.json` na linha ~1185. Premissas do Simulador (peso, reputação, imposto, embalagem, ads) alimentam também os lucros da aba Fornecedores (~linha 1265). |
